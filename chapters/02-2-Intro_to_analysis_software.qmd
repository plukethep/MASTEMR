...
#| eval: true
PISA_2022 %>% 
  select(CNT, PV1MATH, PV1SCIE, PV1READ) %>% 
  mutate(overall_grade = PV1MATH + PV1SCIE + PV1READ)
```

You can also create your own dataframes by combining vectors, note you have to use an equals sign to do this, rather than the arrow assignment:
```{r}
#| echo: true
#| eval: true
my_class_list <- data.frame(
...
Download the `PISA_student_2022_subset.parquet` file from [here](https://emckclac-my.sharepoint.com/:u:/g/personal/k1926273_kcl_ac_uk/EUT13jrQxKJCp64WnH1MG3MBHoDqkzx1NJZIoVpUFHUYvg?e=pi2ZoD) and save it to your computer where your R code file is.

::: callout-note
The PISA student dataset is huge, with over 1,200 pieces of information stored on 600,000+ students. This book uses a subset of this data, but if you do want the full dataset, you can download it from [here](https://emckclac-my.sharepoint.com/:u:/g/personal/k1926273_kcl_ac_uk/EWdsEj9h3CJFmHqA-Qq5jQoBRSKGM_Qm_4Af_xomr2N5kw?e=JNnofI)
:::
...
PISA_2022 <- read_parquet(r"[C:/Users/Peter/code/PISA_2022_student_subset.parquet]")
```
...
PISA_2022 <- read_parquet(r"[PISA_student_2022.parquet]")
...
#| eval: false
library(arrow)
PISA_2022 <- read_parquet("<yourfolder>/PISA_student_2022_subset.parquet")
```
...
#| eval: false
library(tidyverse)
write_parquet(PISA_2022 %>% filter(CNT=="France"), "<yourfolder>/pisa_france_2022.parquet")
```
...
#| eval: false
library(tidyverse)
# saving your results to a file called `summary_stats.csv`
write_csv(my_results, "summary_stats.csv")
```
...
```{r}
#| echo: false
PISA_2022[c("CNTSTUID", "ST004D01T","CNT", "ESCS", "PV1MATH")] %>% head(5)

# PISA_2022 %>% select(where(is.numeric)) %>% View()
...
#| echo: false
#| eval: false
PISA_2022 %>% head(100) %>% pull(ST004D01T)
```
...
You can see one of the `NA`s by just looking at this column:
...
Looking at the structure of this column, we can see it is stored as a `factor`, not as a `numeric`. Factors are a special datatype that use numbers to link to categorical values. For `ST253Q01JA` there are 12 different _levels_ that you can explore using the `str()` and `levels()` commands:
