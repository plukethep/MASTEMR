...
::: callout-tip
An important step in using `chisq.test` is getting the data into the right format to pass to `chisq.test`. As you can see, most of the steps above involve formatting the data so it is in a form the function expects. R tends to use a long format, whereas in the real world we tend to use a wide format.

The wide format looks like this (there are multiple measurements per row and the school names don't repeat):

|          | Avg Math Score | Avg English Score | Avg Sci Score |
|----------|----------------|-------------------|---------------|
| School A | 82             | 91                | 88            |
| School B | 52             | 56                | 57            |
| School C | 23             | 31                | 32            |
...
The `pivot.wider` function switches between the two forms of table (see below)
:::

::: callout-tip
**Using `pivot.wider`**

`pivot.wider` is a useful function for turning the wide forms of tables into long forms (see above). The inputs of pivot wider are a) the names of the columns you want to turn into a single column. For example, to produce the long form of the table of results above, I want to turn the three columns of English, maths and science scores into a single column.

The first information I pass `pivot.wider` is the names of those columns: 'pivot_longer(cols=c("mathssc", "englishsc", "scisc")'. Second, when I stack those values, I will end up with two new columns, one with the names of the subjects, and one with the values of the scores. I tell `pviot.wider` what I want the titles of those columns to be: 'names_to= "subject", values_to= "scores"'.
...
# | B       | 81         | 92            | 78            |
#
# pivot.longer can convert this table into a long form table:
#
...
longresults<- results %>% 
  pivot_longer(cols=c("mathssc", "englishsc", "scisc"),  
                        # indicates we want to stack these 3 columns
  names_to= "subject",  # gives the name of the new, longer label column
  values_to= "scores")  # gives the name of the new, longer values column
...
chidata<-drop_na(chidata)
 ...
 chidata <- PISA_2018 %>%
   ...
   drop_na()
# Above is the approach I took in the video
